...
const useFarmUserInfo = (farmAddress, token0, token1) => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const [rewards, balance] = await Promise.all([farmContract.methods.rewards(account).call(), farmContract.methods.balanceOf(account).call()]);
